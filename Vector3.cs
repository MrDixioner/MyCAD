...
namespace MyCAD {
	public class Vector3 {
		...
		public Vector3 Round(int numDigits) {
			return new Vector3(Math.Round(x, numDigits), Math.Round(y, numDigits), Math.Round(z, numDigits));
		}
		...
		public bool Equals(Vector3 v,double threshold) {
			return (HelperClass.IsEqual(v.X, x, threshold) &&
				HelperClass.IsEqual(v.Y, y, threshold) &&
				HelperClass.IsEqual(v.Z, z, threshold));
		}

        public override bool Equals(object obj) {
			if (obj is Vector3)
				return Equals((Vector3)obj, HelperClass.Epsilon);
			return false;
        }
		...
		public Vector3 Transfer2D(double length,double angle) {
		...
		public Vector3 Scale(Vector3 basePoint,double value) {
			double length = DistanceFrom(basePoint) * value;
